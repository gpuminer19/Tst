...
def init_supabase():
    ...
    try:
        ...
        print(f"Init error (tables may already exist): {e}")
...
async def register_player(data):
    ...
    if existing.data:
        ...
        state = json.loads(player["state"]) if player.get("state") else {}
        return {
            "success": True,
            "data": {
                "ton": player["ton"],
...
# ========== АДМИНКА (остаётся такой же, но данные из Supabase) ==========
...
async def render_admin():
    ...
    total_ton = sum(p["ton"] for p in players.data) if players.data else 0
    total_gpu = sum(p["gpu"] for p in players.data) if players.data else 0
    
    deposits = supabase.table("deposits").select("*").eq("status", "pending").order("created_at", desc=True).limit(30).execute()
    pending_count = len(deposits.data)
    
    top_players = supabase.table("players").select("user_id,name,ton,gpu,friends").order("ton", desc=True).limit(10).execute()
    recent_players = supabase.table("players").select("user_id,name,ton,created_at").order("created_at", desc=True).limit(10).execute()
    ...
    top_html = ""
    for i, p in enumerate(top_players.data):
